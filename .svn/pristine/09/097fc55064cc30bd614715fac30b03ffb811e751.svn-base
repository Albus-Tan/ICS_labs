...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t ovf = ((argA<0) == (argB<0)) && ((val<0) != (argA<0));
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    switch (cond){
        case C_YES:  /* x = 0 */
            ...
            break;
        case C_LE:  /* x = 1 */
            if((GET_SF(cc)^GET_OF(cc))||GET_ZF(cc)) doit = TRUE;
            break;
        case C_L:  /* x = 2 */
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch(icode) {
        ...
        case I_ALU: /* 6:x regA:regB */    
            if (!get_byte_val(sim->m, next_pc, &registers)) {
            ...
            regA = GET_REGA(registers);
            regB = GET_REGB(registers);
            next_pc++;
            if(!NORM_REG(regA) || !NORM_REG(regB)){

                // TODO: err reg id ?

                err_print("PC = 0x%lx, Invalid register id", sim->pc);
                return STAT_INS;
            }
            break;
    ...
    switch (icode) {
      ...
      case I_NOP: /* 1:0 */
    	...
    	break;
      case I_RRMOVQ: {  /* 2:x regA:regB */
        if (cond_doit(sim->cc, ifun)) set_reg_val(sim->r, regB, get_reg_val(sim->r, regA));
        sim->pc = next_pc;
        break;
      }
      case I_IRMOVQ: /* 3:0 F:regB imm */
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        ...
        long_t addr = imm + get_reg_val(sim->r, regB);
        if(!set_long_val(sim->m, addr, get_reg_val(sim->r, regA))){
            err_print("PC = 0x%lx, Invalid data address 0x%.8lx", sim->pc, addr);
    	    return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      ...
      case I_ALU: {  /* 6:x regA:regB */
        long_t result = compute_alu(ifun, get_reg_val(sim->r, regA), get_reg_val(sim->r, regB));
        set_reg_val(sim->r, regB, result);
        sim->cc = compute_cc(ifun, get_reg_val(sim->r, regA), get_reg_val(sim->r, regB), result);
        sim->pc = next_pc;
        break;
      }

      case I_JMP: { /* 7:x imm */
        if(cond_doit(sim->cc, ifun) && imm <= sim->m->len) {  // attention!!! not bigger than len
            sim->pc = imm;
        } else {
            sim->pc = next_pc;
        }
      ...
      case I_CALL: { /* 8:x imm */
        ...
        if(!set_long_val(sim->m, addr, next_pc)){
            err_print("PC = 0x%lx, Invalid stack address 0x%.8lx", sim->pc, addr);
    	    return STAT_ADR;
        }
        // jmp to imm
        sim->pc = imm;
      ...
      case I_RET: {  /* 9:0 */
        long_t add = 0;
        long_t addr = get_reg_val(sim->r, REG_RSP);
        if(!get_long_val(sim->m, addr, &add)){
            err_print("PC = 0x%lx, Invalid stack address 0x%.8lx", sim->pc, addr);
    	    return STAT_ADR;
        }
        sim->pc = add;
      ...
      case I_PUSHQ: /* A:0 regA:F */
        if(regA != REG_RSP) {
            ...
            long_t old_val = get_reg_val(sim->r, regA);
            set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP)-8);
            long_t addr = get_reg_val(sim->r, REG_RSP);
            if(!set_long_val(sim->m, addr, old_val)){
                err_print("PC = 0x%lx, Invalid stack address 0x%.8lx", sim->pc, addr);
    	        return STAT_ADR;
            }
        }
