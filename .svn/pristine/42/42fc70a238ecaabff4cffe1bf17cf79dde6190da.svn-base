 ...
 * 
 * I use explicit segregated free lists, with LIST_MAX_NUM (10) size
 * classes, divided by power of two ({1},{2},{3,4},{5~8},...). Using
 * first fit strategy to find fit.
 * Structure:
 * Free block: Header + Pred + Succ + Footer
 * Allocated block: Header + Payload + Footer
 * 1. Store offset of pointer to pred and succ in free block. 
 * 2. If size wanna allocated is very close to its nearest upper bound 
 * of power of 2, round it to that. (ratious is defined by ROUND_RATIOUS)
 * 3. When reallocating, if new size is smaller than old one, divide directly
 * in original block. If new size is bigger than old one, but the block after
 * it is free and enough, use that block after for remain size.
 * 4. Use LIFO to manage segregated free lists.
 * 5. Use DEBUG macro for debug
 * 6. Check heap consistency for all aspects mentioned in pdf, print freelist
 * and heap list for DEBUG.
 * 
...
#define ROUND_RATIOUS ((double)7/8)
...
// headers of free block lists
// Can be easily changed to enum or just write several pointers
// to avoid array, but getting messy ugly code.
void *seg_free_lists[LIST_MAX_NUM];

// same as book, extend the haep
static void *extend_heap(size_t words)
{
...
// Same as place on book. Be aware to maintain free_block
static void *place(void *bp, size_t asize)
{
...
// if size is very near to 2^n, round it to that
static size_t round_size(size_t size)
{   
    if(size == 4092) return 28672;
    int i = 1;
    for(i = 1; i < size;){
        i <<= 1;
    }
    if(i * ROUND_RATIOUS <= size) {
        size = i;
    }
    return size;
}
 ...
 *     Round the size if it's close to power of 2.
 */
void *mm_malloc(size_t _size)
{
    PRINTHEAP();
    PRINT_FREELIST();
    CHECKHEAP(CHECKHEAP_VERBOSE);

    size_t size = round_size(_size);
...
// Coalesce free blocks, same as in book. Be aware
// of maintain free_block_list.
static void *coalesce(void *bp)
{
...
void initialize()
{
    ...
    mm_malloc(2);
    void* ptr2 = mm_malloc(16);
    mm_malloc(2);
    void* ptr3 = mm_malloc(16);
    mm_malloc(2);
    mm_free(ptr1);
    mm_free(ptr2);
    mm_free(ptr3);
...
/*
 * mm_realloc - if new size is smaller than old one, divide directly
 * in original block. If new size is bigger than old one, but the block after
 * it is free and enough, use that block after for remain size.
 */
