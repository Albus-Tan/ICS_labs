...
word aluA = [
	# add forward here
	E_icode == IOPQ && M_icode == IMRMOVQ && M_dstM == E_srcA : m_valM;
	#
...
word aluB = [
	# add forward here
	E_icode == IOPQ && M_icode == IMRMOVQ && M_dstM == E_srcB : m_valM;
	#
...
word e_valA = [
	# add forward here
	E_srcA == M_dstM && M_icode == IMRMOVQ && E_icode == IRMMOVQ : m_valM;
	#
...
bool F_stall =
	...
	# change here
	# but not for special case load forwarding added
	# E_icode in { IMRMOVQ, IPOPQ } &&
...
bool E_bubble =
	...
	# but not for special case load forwarding added
	# E_icode in { IMRMOVQ, IPOPQ } &&
	# ((E_dstM == d_srcA && !(D_icode in {IRMMOVQ,IOPQ,IPUSHQ})) || (E_dstM == d_srcB && D_icode != IOPQ));
	#
