...
int divpwr2(int x, int n) {
  int signMask = x >> 31;  // pos all 0, neg all 1
...
int isGreater(int x, int y) {
  int sameSign = ((x ^ y) & (1 << 31)) >> 31;  // same all 0, diff all 1
  int differ = x + ~y + 1;  // x - y
  int differSign = !(differ >> 31);  // positive 1, negative 0, equal 1
  int isEqual = !(x ^ y);  // equal 1, not 0
...
int satAdd(int x, int y) {
  int result = x + y;
  int sameSign = ((x ^ y) >> 31);  // same sign all 0, else all 1
  int signChange = ((result ^ x) & (result ^ y)) >> 31;  // result sign change all 1, unchanged all 0
