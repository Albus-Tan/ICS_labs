...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* find register */
    const reg_t *reg_find = find_register(*ptr);
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    *value = strtoll(*ptr, &endptr, 0);
    ...
    // BUGS nees to FIX:
    // not hex and overflow

    if(*value == 0x7fffffffffffffff){  // !!!!! signed long overflow, stroll can not deal with that
        char* p = *ptr;
        p += 2;
        byte_t lowest_4_bits = 0;
        for(int i = 0; i < 16; ++i){
            lowest_4_bits = 0;
            if(*p >= '0' && *p <= '9'){
                lowest_4_bits = *p - '0';
            } else if (*p >= 'a' && *p <= 'f') {
                lowest_4_bits = *p - 'a' + 10;
            } else if (*p >= 'A' && *p <= 'F') {
                lowest_4_bits = *p - 'A' + 10;
            }
            *value = (*value << 4) + lowest_4_bits;
            p += 1;
        }
    }
